            ...
            <div className="w-full max-w-xl space-y-0">
                ...
                <div className="rounded-2xl px-6 pt-10 pb-8 -mt-12 text-black bg-white border border-gray-200 shadow-md overflow-hidden relative">
                    <div className="relative z-10 flex flex-col items-center">
                        ...
                        <p className="mb-6 text-center text-slate-500 dark:text-slate-400 font-medium max-w-xs mx-auto">
                            AI will automatically process your schedule and find the fastest route to your classes!
                        </p>
                        ...
                        >
                            ...
                            <div
                                className={`absolute inset-0 bg-blue-50 transition-all duration-300 ease-out z-0 ${isAnalyzing ? 'opacity-100' : 'opacity-0'}`}
                                style={{ width: isAnalyzing ? `${uploadProgress}%` : '0%' }}
                            />
                ...
                <div className="flex flex-col md:flex-row items-stretch justify-between gap-4 w-full mt-6">
                    {/* Left Button: Browse Map */}
                    <Link href={parsed.length > 0 ? `/map?schedule=${encodeURIComponent(JSON.stringify(parsed.map(p => ({
                        course: p.course,
                        room: p.room,
                        time: p.time,
                        type: p.type,
                        ...
                        >
                            <div className="flex items-center justify-center gap-3">
                                ...
                                <span>{parsed.length > 0 ? 'View All Classes' : 'Browse Map'}</span>
                            </div>
                        </ThreeDButton>
                    </Link>

                    {/* Right Button: Supercharge */}
                    <Link href="/supercharge" className="w-full md:flex-1 flex">
                        <ThreeDButton
                            variant="accent"
                            className="w-full h-full text-lg"
                            glowEffect={true}
                        >
                            <span>Supercharge Your Schedule</span>
                        </ThreeDButton>
                    </Link>
                </div>
